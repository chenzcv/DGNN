...
class Data:
  def __init__(self, sources, destinations, timestamps, edge_idxs, labels, sample=1, random_sample=False, exp_id=0, sample_type=None):
    if random_sample:
      ...
      sources = sources[random_index]
      destinations = destinations[random_index]
...
def get_data(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False, sample=0, random_sample=False, exp_id=0, sample_type=None):
  ...
  train_data = Data(sources[train_mask], destinations[train_mask], timestamps[train_mask],
                    edge_idxs[train_mask], labels[train_mask], sample=1)
  ...
  val_data = Data(sources[val_mask], destinations[val_mask], timestamps[val_mask],
                  edge_idxs[val_mask], labels[val_mask], sample=1)

  test_data = Data(sources[test_mask], destinations[test_mask], timestamps[test_mask],
                   edge_idxs[test_mask], labels[test_mask], sample=1)

  # validation and test with edges that at least has one new node (not in training set)
  new_node_val_data = Data(sources[new_node_val_mask], destinations[new_node_val_mask],
                           timestamps[new_node_val_mask],
                           edge_idxs[new_node_val_mask], labels[new_node_val_mask], sample=1)

  new_node_test_data = Data(sources[new_node_test_mask], destinations[new_node_test_mask],
                            timestamps[new_node_test_mask], edge_idxs[new_node_test_mask],
                            labels[new_node_test_mask], sample=1)
