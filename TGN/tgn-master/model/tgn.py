...
class TGN(torch.nn.Module):
    def __init__(self, neighbor_finder, node_features, edge_features, device, n_layers=2,
        ...
        self.refresh_history_memory = refresh_history_cache
        ...
        if self.use_memory or self.refresh_history_memory:
            ...
            self.memory = Memory(n_nodes=self.n_nodes,
                                 ...
                                 device=device,
                                 refresh_history_cache=self.refresh_history_memory)
    ...
    def compute_temporal_embeddings(self, source_nodes, destination_nodes, negative_nodes, edge_times,
        ...
        nodes = np.concatenate([source_nodes, destination_nodes, negative_nodes])  # [bs*3]
        ...
        if self.use_memory or self.refresh_history_memory:
            ...
            source_time_diffs = torch.LongTensor(edge_times).to(self.device) - last_update[
                source_nodes].long()
            source_time_diffs = (
                                        source_time_diffs - self.mean_time_shift_src) / self.std_time_shift_src  # normalization
        ...
        node_embedding, attention_weight = self.embedding_module.compute_embedding(memory=memory,
                                                                                   source_nodes=nodes,
        ...
        if self.use_memory:
            ...
            # obtain message (source and destination)
            ...
            unique_destinations, destination_id_to_messages = self.get_raw_messages(destination_nodes,
                                                                                    ...
                                                                                    source_node_embedding,
                                                                                    edge_times, edge_idxs)
            if self.memory_update_at_start:  # store the message into raw store
    ...
    def compute_edge_probabilities(self, source_nodes, destination_nodes, negative_nodes, edge_times,
        ...
        return pos_score.sigmoid(), neg_score.sigmoid(), self.affinity_score.h, attention_weight
