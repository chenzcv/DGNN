...
class GraphEmbedding(EmbeddingModule):
    ...
    def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
        ...
        timestamps_torch = torch.unsqueeze(torch.from_numpy(timestamps).float().to(self.device), dim=1)  # [bs*3, 1]
        ...
        if n_layers == 0:
            ...
            # diff_time_neighbor = np.mean(timestamps.reshape(-1, 1).repeat(10, axis=1) - edge_times)
            # # diff_time_neighbor = np.mean(timestamps[:] - edge_times[:, 0])
            # self.time_diff_nb.append(diff_time_neighbor)
            # if self.count == 50:
            #     fig = plt.figure()
            #     plt.plot(range(len(self.time_diff_nb)), self.time_diff_nb, 'b')
            #     plt.savefig('./figures/time_diff_all_1600.jpg')
            #     plt.show()
            # self.count += 1
            ...
            source_embedding, attention_weight = self.aggregate(n_layers, source_node_features,
            ...
            return source_embedding, attention_weight
...
class GraphAttentionEmbedding(GraphEmbedding):
    ...
    def aggregate(self, n_layer, source_node_features, source_nodes_time_embedding,
        ...
        source_embedding, attention_weight = attention_model(source_node_features,
                                                             source_nodes_time_embedding,
                                                             neighbor_embeddings,
                                                             edge_time_embeddings,
                                                             edge_features,
