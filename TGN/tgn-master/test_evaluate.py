...
parser.add_argument('--sample', type=int, default=1, help='only for path')
...
new_node_test_data = get_data(DATA,
                              ...
                              random_sample=args.random_sample,sample_type=args.sample_type)
...
results_path = "test_evaluation/{}_sample{}_mean/result.pkl".format(args.prefix, args.sample)
parent_path = "test_evaluation/{}_sample{}_mean/".format(args.prefix, args.sample)
...
checkpoint1 = f'./saved_models/{args.data}-tgn-attn-sample{args.skip}-{args.sample_type}-0-memory.pth' if args.use_memory \
    else f'./saved_models/{args.data}-tgn-attn-sample{args.skip}-{args.sample_type}-0.pth'
checkpoint2 = f'./saved_models/{args.data}-tgn-attn-sample{args.skip}-{args.sample_type}-1-memory.pth' if args.use_memory \
    else f'./saved_models/{args.data}-tgn-attn-sample{args.skip}-{args.sample_type}-1.pth'

model1 = torch.load(checkpoint1, map_location=torch.device('cpu'))
model2 = torch.load(checkpoint2, map_location=torch.device('cpu'))
model_final = load_model(model1,model2)

tgn.load_state_dict(model_final)
tgn.eval()
